			...
			<h3>Users Management</h3>
				...
				echo '
					<form id="form" name="form" method="post" action="admin.php">							
						<center>List of Administrators<select onchange="showUser(this.value)" name="admin" id="admin"></center>
							<option value="-1">New</option>';
								$suite=array();
								$suite=$conn->Select('SELECT * FROM users order by username');
					...
					if(isset($_POST['update']))	 	
					{
						...
						try
							{
								$retour = $conn->update('UPDATE users SET username=?, password=?, email=?, blocked=? where id =\''.$_POST['admin'].'\'', $update);
							}
							catch (Exception $e)
							{
								echo 'Please verify your inputs';
					...
					if(isset($_POST['block']))	 	
					{
						...
						try
							{
								$retour = $conn->update('UPDATE users SET blocked=1 where id =\''.$_POST['admin'].'\'', $update);
							}
							catch (Exception $e)
							{
					...
					if(isset($_POST['unblock']))	 	
					{
						...
						try
							{
								$retour = $conn->update('UPDATE users SET blocked=0 where id =\''.$_POST['admin'].'\'', $update);
							}
							catch (Exception $e)
							{
								echo 'Please verify your inputs, and put a new password';
					...
					if(isset($_POST['delete']))	 	
					{
						...
						try
							{
								$retour = $conn->Delete('DELETE FROM users where id =\''.$_POST['admin'].'\'', $update);
